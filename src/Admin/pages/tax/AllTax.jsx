...
function AllTax() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/admin/all-tax?page=${currentPage}&limit=${limit}&search=${searchTerm}`);
      setData(response.data.Tax);
  ...
  const handleDelete = (categoryId) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axiosInstance.delete(`/admin/delete-tax/${categoryId}`)
          .then(() => {
            // Refresh the data or update the state after successful deletion
            fetchData();
            toast.success("Deleting Taxes Success!");
          })
          .catch((error) => {
        ...
        <div className="py-4">
          ...
          <div className="d-flex justify-content-between w-100 flex-wrap">
            <div className="mb-3 mb-lg-0">
              <h1 className="h4"> All Charges</h1>
            </div>
            <div>
              <Link
                to="/add-taxes"
                  ...
                  <tbody>
                    {data.map((row) => (
                      <tr key={row._id}>
                        ...
                        <td>
                          <Link to={`/edit-taxes/${row._id}`} className="btn btn-primary me-2">
                            Edit
                          </Link>
                          <button
