...
function AllAttribute() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/admin/all-attribute-fillter?page=${currentPage}&limit=${limit}&search=${searchTerm}`);
      setData(response.data.Attribute);
      setEmployees(response.data.Attribute); // Use response.data directly
  ...
  const handleDelete = (categoryId) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axiosInstance.delete(`/admin/delete-attribute/${categoryId}`)
          .then(() => {
            // Refresh the data or update the state after successful deletion
            fetchData();
            toast.success("Deleting attribute success!");
          })
          .catch((error) => {
        ...
        <div className="py-4">
          ...
          <div className="d-flex justify-content-between w-100 flex-wrap">
            <div className="mb-3 mb-lg-0">
              <h1 className="h4"> All Attribute</h1>
            </div>
            <div>
              <Link
                to="/add-attribute"
