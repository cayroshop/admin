...
const AllBlogs = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.get(`/admin/all-blogs?page=${currentPage}&limit=${limit}&search=${searchTerm}`);
    ...
    const handleDelete = (categoryId) => {
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axiosInstance.delete(`/admin/delete-blog/${categoryId}`)
                    .then(() => {
                                    ...
                                    <tbody>
                                        {data.map((row) => (
                                            <tr key={row._id}>
                                                ...
                                                <td>
                                                    <Link to={`/edit-blog/${row._id}`} className="btn btn-primary me-2">
                                                        Edit
                                                    </Link>
                                                    <button
